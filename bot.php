...
animasi($m."╚{$p}═══════════════════════════════════════════════{$m}╝\n");
animasi($p." {$k}◆ {$p}Facebook    : ナンダ\n");
animasi($p." {$k}◆ {$p}Github : https://github.com/bambank1\n");
...
if($info=="success")
{
    ...
    {
        animasi($r.$p."[{$m}x{$p}] {$m}tunggu {$p}1 jam\n");
        for($x=3600;$x>0;$x--)
            {
                echo "\r \r"; //timer
                echo$p."[{$k}#{$p}] {$p}hitung mundur ".$h.$x."  ";
                echo "\r \r";
                sleep(1);
            }
        goto mengulang;
